...
void stepper_set_next_step(int direction, step_mode_t mode)
{
    ...
    if (phase_index < 0 && (mode == WAVE_DRIVE|| mode == FULL_STEP) ) {
        phase_index = 3;
    } else if (phase_index > 3 && (mode == WAVE_DRIVE || mode == FULL_STEP)){
        phase_index = 0;
     }
    else if( phase_index < 0 && mode == HALF_STEP){
    	phase_index = 7;
    }
    else if ( phase_index > 7 && mode == HALF_STEP){
    	phase_index = 0;
    }
